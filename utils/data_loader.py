...
class DataLoader:
	...
	def load(self, dataset):
		...
		for name in pbar:
			...
			all_filesList = pd.read_csv('/data/liuqinghua/code/ts/TSAD-AutoML/AutoAD_Solution/file_list/all_file_list.csv')
			all_filesList_name = all_filesList[all_filesList['Dataset']==name]
			for index, row in tqdm(all_filesList_name.iterrows(), leave=True, desc="Process", ncols=75):
			# for fname in glob.glob(os.path.join(self.data_path, name, '*.out')):
				fname = self.data_path+row['Dataset'] + '/' + row['File_name']
				curr_data = pd.read_csv(fname, header=None).dropna().to_numpy()
				...
				if not np.all(curr_data[0, 1] == curr_data[:, 1]):
					x.append(curr_data[:, 0].astype(float))
