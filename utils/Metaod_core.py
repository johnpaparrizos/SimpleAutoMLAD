...
class MetaODClass(object):
    ...
    def train(self, meta_features, valid_meta=None, n_iter=10,
        ...
        self.user_vecs = meta_features_scaled       # (n_users, n_factors) --> (number_ts, PCA_n_components)
        # print('self.user_vecs: ', self.user_vecs.shape)

        self.item_vecs = np.random.normal(scale=1. / self.n_factors,
                                          size=(self.n_items, self.n_factors))   # (n_items, n_factors)
        # print('self.item_vecs: ', self.item_vecs.shape)
        ...
        while ctr <= n_iter:
            ...
            self.regr_multirf = MultiOutputRegressor(RandomForestRegressor(n_estimators=n_estimators, max_depth=max_depth, n_jobs=-1))
            ...
            ndcg_s = []
            for w in range(self.ratings.shape[0]):      # n_users
            ...
            for w in range(self.valid_ratings.shape[0]):
                ndcg_s.append(ndcg_score([self.valid_ratings[w, :]], [np.dot(meta_valid_scaled_new[w, :], self.item_vecs.T)],
                                         k=self.n_items))
            ...
            for h in train_indices:
                ...
                uh = self.user_vecs[h, :].reshape(1, -1)    # (n_factors, )
                # print(uh.shape)
                grads = []

                for i in range(self.n_models):      # n_items
                    # outler loop
                    vi = self.item_vecs[i, :].reshape(-1, 1)    # (n_factors, )
                    phis = []
                    rights = []
                    rights_v = []
                    # remove i from js 
