...
class ModelExecutioner:
    ...
    def train_one_epoch(self, epoch_index, tb_writer, training_loader):
        ...
        for i, (inputs, labels) in loop:
            ...
            # print('inputs: ', inputs.shape)      # torch.Size([256, 1, 1024])
            # print('labels: ', labels.shape)      # torch.Size([256, 23])
    ...
    def evaluate(self, dataloader):
        ...
        with torch.no_grad():
            ...
            for i, (inputs, labels) in loop:
                ...
                all_loss.append(loss.item())
                all_ndcg.append(ndcg_score(labels.detach().cpu().numpy(), outputs.detach().cpu().numpy()))

                # Report on progress bar
                if i % 10 == 9:
                    loop.set_postfix(
                        val_loss=np.mean(all_loss),
                        val_ndcg=np.mean(all_ndcg),
                    )

        return np.mean(all_loss), np.mean(all_ndcg)
    ...
    def train(self, n_epochs, training_loader, validation_loader, verbose=True, model_name=None):
        ...
        for epoch in range(n_epochs):
            ...
            avg_val_loss, avg_val_ndcg = self.evaluate(validation_loader)


            # Epoch reporting
            print(
                "Epoch [{}/{}] {:.2f}secs : val_ndcg: {:.3f}, loss: {:.3f}, val_loss: {:.3f}"\
                .format(epoch, n_epochs, perf_counter()-tic, avg_val_ndcg, avg_loss, avg_val_loss)
            )
            
            # Log the running loss averaged per batch
            writer.add_scalars('Training vs. Validation Accuracy',
                {'Training': avg_acc, 'Validation': avg_val_ndcg},
                epoch + 1
            )
            ...
            # # Early stopping
            # if (epoch > 3 and early_stopper.early_stop(avg_val_loss)) or ((perf_counter()-tic) > 70000):
            #     break
