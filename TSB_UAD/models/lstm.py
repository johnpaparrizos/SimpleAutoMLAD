...
class lstm:
    def __init__(self, slidingwindow = 100, predict_time_steps=1, contamination = 0.1, epochs = 10, patience = 10, verbose=0, hidden_dim=8, activation='relu'):
    ...
    def fit(self, X_clean, X_dirty, ratio = 0.15):
        ...
        model = Sequential()
        model.add(LSTM(units=self.hidden_dim, input_shape=(self.slidingwindow, 1),
        ...
        for layer_idx in range(self.n_hidden_layer):
            model.add(LSTM(units=self.hidden_dim, input_shape=(self.hidden_dim, 1),
                             dropout=self.dropout_rate, activation=self.activation,
                             return_sequences=bool(layer_idx < self.n_hidden_layer - 1)))
        model.add(Dense(units=self.predict_time_steps, input_shape=(self.hidden_dim, 1), activation=None))
