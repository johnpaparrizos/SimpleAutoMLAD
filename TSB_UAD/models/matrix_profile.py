...
class MatrixProfile():
    def __init__(self, window = 100, cross_correlation=False):
        self.window = window
    ...
    def fit(self, X, y=None):
        ...
        if self.cross_correlation:
            self.profile = mp.compute_mpx(X, windows=self.window, cross_correlation=self.cross_correlation, n_jobs=-1)
        else:
